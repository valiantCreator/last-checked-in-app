...
function ExportCalendarModal({ isOpen, onClose, onGenerateFiles }) {
  ...
  const handleExportClick = () => {
    ...
    const filesToDownload = Object.values(files).filter(
      (file) => file !== null
    );
      ...
      <div className={styles.modalContent} onClick={(e) => e.stopPropagation()}>
        ...
        {modalView === "options" && (
          <>
            ...
            <p>
              Select what you'd like to export. This will download an .ics file
            ...
            <div className={styles.modalOption}>
              <label>
                <input
                  ...
                  onChange={(e) => setExportBirthdays(e.target.checked)}
                />
                Export Birthdays
              </label>
            </div>
            <div className={styles.modalOption}>
              <label>
                <input
                  type="checkbox"
                  checked={exportCheckins}
                  onChange={(e) => setExportCheckins(e.target.checked)}
                />
                Export Check-in Reminders
              </label>
            </div>
            <fieldset
              className={styles.timeWindowFieldset}
              disabled={!exportCheckins}
            >
            ...
            <div className={styles.modalActions}>
              ...
              <button className="button-primary" onClick={handleExportClick}>
                Export
              </button>
            </div>
          </>
          ...
          <>
            ...
            <div className={styles.downloadLinks}>
              {generatedFiles.birthdays && (
                <button
                  className={`button-primary ${styles.downloadLink}`}
                  onClick={() => triggerDownload(generatedFiles.birthdays)}
                >
              ...
              {generatedFiles.checkins && (
                <button
                  className={`button-primary ${styles.downloadLink}`}
                  onClick={() => triggerDownload(generatedFiles.checkins)}
                >
