...
* **Multi-User Push Notifications:** An hourly scheduled job on the backend (node-cron) is the single source of truth for notifications. It queries users whose preferred notification hour matches the current UTC hour, then performs a fast, indexed lookup on the pre-calculated `next_checkin_date` column to find overdue contacts. It sends a single, dynamic summary push notification.  
